...
**2. Advisor role: overlay only, diff-gated.** Advisors keep the full toolset, bash included. Read-only is a property of the checkout, not of the toolset, so empirical verification (running experiments, scratch files, cleanup chains) stays possible and becomes safe by default. The advisor namespace gets no socket bind (a dispatch binds only the socket file `<runtimeDir>/daemon.sock` read-write; the advisor binds nothing from `runtimeDir`), so the daemon's Unix socket is unreachable. Network stays shared with every other role. `--unshare-net` was tried for advisor and reverted when it blocked the worker CLI's own outbound access to its model provider. On exit, taskferry computes the changeset from `<upper>` by running `git diff` inside one short-lived bwrap that re-mounts the same overlay read-only (this resolves deletions and overlayfs whiteouts without custom parsing). A dispatch changeset becomes pending for accept/reject. An advisor changeset auto-rejects after extraction and never reaches an accept path.
