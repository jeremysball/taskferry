...
**Status:** Accepted (direction). Mechanism verified on the target host; implementation pending a small rework of `src/sandbox.js`.
...
Taskferry already sandboxes every executor spawn with bwrap (`src/sandbox.js`, single call site `src/tasks.js:1683`): read-only root, tmpfs `/tmp`, a deny-list tmpfs over the secret paths (`~/.ssh`, `~/.aws`, gcloud, gh, gnupg, the taskferry state dir), `--unshare-all --share-net --die-with-parent`. It bind-mounts the target directory read-write: `--bind directory directory`.
...
**1. Every ferry gets a copy-on-write overlay on its target directory.** In `buildBwrapArgs()`, the `--bind directory directory` line becomes, on overlay-capable hosts, `--overlay-src <directory> --overlay <upper> <work> <directory>` (bwrap ≥ 0.8; argument order is RWSRC WORKDIR DEST; syntax and semantics verified on the target host, see Evidence). The worker sees the real path. All writes and deletes land in `<upper>`; the underlying tree is read-only by construction. `<upper>` and `<work>` are per-task unique (`/tmp/taskferry-cow-<task-id>/{upper,work}`), never shared between concurrent ferries: a shared overlayfs workdir corrupts the mount, and a shared upper would blend two workers' change-sets into one undecidable diff. Both live under `/tmp`, a tmpfs on the target host, bounding a runaway writer by RAM rather than disk, and are destroyed on reject or after apply.

**2. Advisor role: overlay only, diff-gated, network off.** Advisors keep the full toolset, bash included. Read-only is a property of the checkout, not of the toolset, so empirical verification (running experiments, scratch files, cleanup chains) stays possible and becomes safe by default. The advisor namespace additionally gets `--unshare-net`, reversing v1's `--share-net` for this role. On exit, taskferry computes the changeset from `<upper>` by running `git diff` inside one short-lived bwrap that re-mounts the same overlay read-only (this resolves deletions and overlayfs whiteouts without custom parsing), presents it for accept/reject, and discards `<upper>` on reject. An advisor has no path to persist a write.
...
- Working invocation: `bwrap --unshare-user --ro-bind / / --dev /dev --proc /proc --overlay-src <repo> --overlay <upper> <work> <repo> <cmd>`.
