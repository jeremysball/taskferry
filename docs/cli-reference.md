...
| `--rw-dirs <path,path,...>` | Extra directories bound read-write inside the sandbox for this dispatch, on top of the auto-detected git-common-dir for a worktree and any config-level `rwDirs`; see [security.md](security.md). **`/tmp` needs this too** — the sandbox mounts a fresh, empty `--tmpfs /tmp`, so any path under `/tmp` that isn't `--directory`, `runtimeDir`, or an `--rw-dirs` entry is invisible inside the sandbox even though it exists on the host. All three layers (this flag, `TASKFERRY_RW_DIRS`, config `rwDirs`) union rather than replace. |
| `--ro-dirs <path,path,...>` | Extra directories bound **read-only** inside the sandbox for this dispatch — a review-only worker that should read several repos but edit none. Resolved through the same protected-mount safety check as `.taskferry.toml`'s `read_only_paths`: an entry that doesn't exist on the host, or that overlaps a protected mount, is skipped and reported. Unions with `TASKFERRY_RO_DIRS` and config `roDirs`. If a path also appears in the read-write set, read-write wins with a warning. |
| `--allowed-dirs <path,path,...>` | **Deprecated** alias for `--rw-dirs` (same read-write behavior). Emits a deprecation warning when used; will be removed in the next major release. |
