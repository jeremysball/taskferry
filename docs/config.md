...
| `taskRetentionDays` | `TASKFERRY_TASK_RETENTION_DAYS` | number | `90`; `0` disables retention entirely. The daemon sweeps `tasks.json` at boot, evicting tasks that are both terminal (`done`, `crashed`, `cancelled`, `unknown`) and older than this window. Evicted records are archived as NDJSON under `<stateDir>/archive/tasks-pruned-<stamp>.ndjson`, never deleted. `queued` and `running` tasks are never evicted at any age, and neither is a record carrying no parseable `startedAt`/`endedAt`. Run the same sweep on demand with `taskferry prune` (`--keep-days` overrides this value for that one call). |
