...
`src/tasks.js` (5112 lines) is the largest file by a wide margin and does
...
| `doctor-stats.js` | 135 | `computeDoctorStats()`: pure aggregation of `task.list` rows into the `doctor --stats` report shape — status mix (overall/24h/7d), per-model dispatch/rate breakdown, failure-reason histogram, unknown backlog, and 24h crash-rate trend. |
