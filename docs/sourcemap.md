...
`src/tasks.js` (2819 lines) is the largest file by a wide margin and does
...
| `changeset.js` | 270 | Overlay path helpers, pre-dispatch HEAD resolution (unborn HEAD anchors on the empty tree), git/non-git diff extraction from the CoW overlay (fail-closed on extraction failure), changeset apply (git apply for git targets, in-sandbox rsync --delay-updates for non-git), and overlay cleanup (plain removal with a containment guard refusing roots outside the overlay tmp root). |
