...
| `daemon-server.js` | 189 | The daemon's connection layer, extracted from `daemon.js`: `createDaemonServer` (socket lifecycle, `cleanup`, the per-line request dispatch loop via `dispatchRequest`), `subscribeRequest` (event.subscribe bookkeeping), `dispatchRequest` (parse, SERVER_BUSY gate, in-flight tracking, `maybeRestart` in `finally`), `deliverEvent`/`eventPayload` (event fan-out including `activityVariants` summary-variant selection), `syncActivitySubscriptions`, `makeClose` (graceful shutdown + socket unlink), and `makeMaybeRestart` (deferred-until-idle self-restart). Exports the `MAX_BUFFER_BYTES` constant. `dispatchRequest`/`createDaemonServer` receive `invoke`/`responseError` from daemon.js via deps (no circular import). |
