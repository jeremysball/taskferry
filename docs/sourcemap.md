...
| `state-lock.js` | 133 | `withFileLock()`: synchronous, `Atomics.wait`-based cross-process exclusive lock; guards the daemon auto-start race and every `persistTask()` write (dispatch/cancel/settlement — the request hot path). `withFileLockAsync()` shares the same synchronous `acquireLock()`/`releaseLock()` primitives but awaits an async critical section — used by `daemon.js` to hold the lock across its socket-prepare-then-bind sequence, which spans an async health-check round trip. Acquisition itself is still a synchronous, thread-blocking `Atomics.wait` loop with no yield to the event loop, so contending on the same lock path *within one process* can starve the very continuation that would release it — harmless in production (each real lock contender is a separate OS process with its own event loop) but means a same-process concurrent-contention scenario is untestable here; see `state-lock.test.js`'s note above the `withFileLockAsync()` suite. |
