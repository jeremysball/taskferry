...
`src/tasks.js` (3449 lines) is the largest file by a wide margin and does
...
| `config.js` | 83 | `loadConfig()`: reads/validates `config.json` against `CONFIG_FIELD_TYPES`, rejects unrecognized keys. |
