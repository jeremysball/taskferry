...
| Where is a pending overlay changeset accepted or rejected? | `tasks.js`'s `accept()`/`reject()`; `changeset.js`'s `applyChangeset()`/`cleanupOverlay()` |
