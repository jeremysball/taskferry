...
| `config.js` | 83 | `loadConfig()`: reads/validates `config.json` against `CONFIG_FIELD_TYPES`, rejects unrecognized keys. `advisorContextChars` controls advisor's auto-attached context budget. |
