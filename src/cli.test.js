...
test("runs init with the literal cwd without connecting to the daemon", async () => {
  const capture = capturedIo();
  const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "taskferry-cli-init-"));
  ...
  const result = await runCli(["init"], {
    ...
    init: async (directory) => {
      ...
      return { path: path.join(directory, ".taskferry.toml"), written: true, checkCommand: null };
    },
    connectClient: async () => { throw new Error(setupMustNotConnect); },
    resolveWorkspaceRoot: () => { throw new Error("init must not resolve the workspace root"); },
  });

  assert.equal(result.exitCode, 0);
  assert.equal(receivedDirectory, cwd);
  assert.equal(capture.output().value.written, true);
});

test("surfaces init failures without connecting to the daemon", async () => {
  const capture = capturedIo();
  const result = await runCli(["init"], {
    io: capture.io,
  ...
  assert.equal(result.exitCode, 1);
  assert.equal(capture.output().value.error, "boom");
});
