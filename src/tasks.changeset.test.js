...
describe("changeset extraction at settlement", () => {
  test("extracts a diff and leaves changesetStatus pending for a settled dispatch with an active overlay", () => {
    ...
    const directory = mkdtempTracked("axi-extract-dir-");
    const overlayTmpRoot = mkdtempTracked("axi-extract-tmp-");
    let child;
    const mgr = makeManager({
  ...
  test("auto-rejects and cleans up an advisor's changeset at settlement", async () => {
    ...
    const overlayTmpRoot = mkdtempTracked("axi-advisor-extract-tmp-");
    let child;
    const mgr = makeManager({
      spawnFn: (_cmd, _args) => { child = fakeChild(); return child; },
      sandboxEnabled: true,
  ...
  test("re-mounts persisted git-common-dir sub-overlays during extraction", () => {
    const directory = mkdtempTracked("axi-extract-git-dir-");
  ...
  test("auto-resolves a zero-change extraction to accepted and cleans up immediately", () => {
    let cleanedRoot = null;
    const directory = mkdtempTracked("axi-empty-extract-dir-");
    const overlayTmpRoot = mkdtempTracked("axi-empty-extract-tmp-");
    let child;
    const mgr = makeManager({
      spawnFn: (_cmd, _args) => { child = fakeChild(); return child; },
      sandboxEnabled: true,
  ...
  test("extracts a changeset for a cancelled task too", () => {
    const directory = mkdtempTracked("axi-cancel-extract-dir-");
    let child;
    const mgr = makeManager({
      spawnFn: (_cmd, _args) => { child = fakeChild(); return child; },
      killFn: () => {},
  ...
  test("records extraction errors and keeps the overlay for recovery", () => {
    const directory = mkdtempTracked("axi-failed-extract-dir-");
    const overlayTmpRoot = mkdtempTracked("axi-failed-extract-tmp-");
...
describe("changeset extraction at settlement: stale-base drift settlement (taskferry#261)", () => {
  test("a recovered stale-base drift settles pending (git target) as normal and stamps audit fields", () => {
    const directory = mkdtempTracked("axi-drift-recovered-dir-");
    const overlayTmpRoot = mkdtempTracked("axi-drift-recovered-tmp-");
    let child;
  ...
  test("an unrecovered (genuinely conflicting) stale-base drift auto-rejects and releases the overlay", () => {
    const directory = mkdtempTracked("axi-drift-conflict-dir-");
    const overlayTmpRoot = mkdtempTracked("axi-drift-conflict-tmp-");
    let cleanedRoot = null;
    let child;
    let revParseCount = 0;
    const mgr = makeManager({
  ...
  test("a drift the scratch-worktree probe could not evaluate (git worktree add failure) settles pending with the infra failure recorded", () => {
    const directory = mkdtempTracked("axi-drift-noeval-dir-");
    const overlayTmpRoot = mkdtempTracked("axi-drift-noeval-tmp-");
    let child;
    let revParseCount = 0;
    const mgr = makeManager({
      spawnFn: (_cmd, _args) => { child = fakeChild(); return child; },
...
describe("changeset extraction at settlement: overlay-mount-busy reclassification", () => {
  test("reclassifies a real no_output_timeout_dead_spawn crash as overlay_mount_busy when the bwrap overlay-busy message is the real cause", async () => {
    const directory = mkdtempTracked("axi-overlay-busy-dir-");
    const overlayTmpRoot = mkdtempTracked("axi-overlay-busy-tmp-");
    const bwrapMessage =
  ...
  test("an unrelated extraction error does not touch failureReason", () => {
    const directory = mkdtempTracked("axi-overlay-unrelated-dir-");
    const overlayTmpRoot = mkdtempTracked("axi-overlay-unrelated-tmp-");
    let child;
    const mgr = makeManager({
      overlayTmpRoot,
...
describe("boot-time sweep of orphaned prompt scratch files in PROMPT_DIR", () => {
  ...
  test("removes prompt files whose task id is not in the loaded task set", () => {
    const stateDir = mkdtempTracked(AXI_TASKS_ORPHAN);
...
describe("sweepOrphanedOverlays()", () => {
  test("removes an overlay directory whose task id is unknown to this manager (crash before extraction ever ran)", () => {
    const overlayTmpRoot = mkdtempTracked("axi-orphan-tmp-");
  ...
  test("a legacy persisted task (overlayDirs.tmpRoot === undefined, predating creation-time tmpRoot persistence) gets its containment root migrated to the pre-upgrade os.tmpdir() default, not today's overlayTmpRoot -- and the sweep finds/cleans it there", () => {
    ...
    // /tmp".
    const overlayTmpRoot = mkdtempTracked("axi-legacy-new-root-");
  ...
  test("does nothing when overlayTmpRoot doesn't exist or is empty", () => {
    const overlayTmpRoot = mkdtempTracked("axi-orphan-empty-");
  ...
  test("sweeps a resolved task overlay under its recorded non-live tmpRoot and persists clearing overlayDirs", () => {
    const stateDir = mkdtempTracked("axi-orphan-resolved-state-");
    const overlayTmpRoot = mkdtempTracked("axi-orphan-resolved-overlay-");
    const liveOverlayTmpRoot = mkdtempTracked("axi-orphan-live-overlay-");
  ...
  test("two managers with distinct runtimeDirs and no explicit overlayTmpRoot never collide on the same overlay namespace (taskferry#286)", () => {
    ...
    const runtimeDirA = mkdtempTracked("axi-runtime-a-");
    const runtimeDirB = mkdtempTracked("axi-runtime-b-");
    const stateDirA = mkdtempTracked(AXI_TASKS_TEST_DIR);
    const stateDirB = mkdtempTracked(AXI_TASKS_TEST_DIR);
    const mgrA = trackManager(createTaskManager({
      stateDir: stateDirA,
      runtimeDir: runtimeDirA,
      sandboxEnabled: false,
      cacheDir: mkdtempTracked("axi-cache-a-"),
      spawnFn: () => { throw new Error("not used"); },
      killFn: () => {},
      listModelsFn: () => `${DEFAULT_SUMMARY_MODEL}\n`,
    }));
    const mgrB = trackManager(createTaskManager({
      ...
      sandboxEnabled: false,
      cacheDir: mkdtempTracked("axi-cache-b-"),
      spawnFn: () => { throw new Error("not used"); },
      killFn: () => {},
      listModelsFn: () => `${DEFAULT_SUMMARY_MODEL}\n`,
    }));
