...
export function createTaskManager({
  ...
  async function summarizeActivity(taskId, maxWords, previousActivity) {
    ...
    try {
      const firstStarted = await summarizeTask(taskId, { maxWords, previousActivity, allowPromptFallback: true, respectConcurrencyReserve: true });
  ...
  async function advisor({ prompt, directory, model, variant, sessionId, timeoutMs, executor, env } = {}) {
    ...
    let dispatched;
    try {
      dispatched = dispatch({ model, variant, executor, env, prompt: /** @type {string} */ (prompt), directory: /** @type {string} */ (directory), sessionId: resolved.sessionId, role: "advisor" });
    } catch (err) {
